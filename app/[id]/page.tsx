...
async function DetailsPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="mb-24 md:mx-16 md:mt-[85px] xl:max-w-6xl xl:mx-auto">
      <h1 className="hidden font-semibold text-2xl md:block mb-4">
        {data.title}
      </h1>
      <div className="relative">
        <ImageSwiper images={data.images} swiperStyle="h-80 md:h-96" />
        <Link
          href="/"
          className="fixed z-10 top-4 left-4 inline-block w-[45px] h-[45px] rounded-full overflow-hidden md:hidden"
        >
          <IoChevronBackCircle
            size={45}
            className="relative z-30 text-buttonPrimary"
          />
        ...
        <FavoriteButton
          listingId={params.id}
          iconSize="30"
          className="fixed top-4 right-4 z-10 md:absolute bg-buttonPrimary rounded-full p-1"
        />
      </div>
      <div className="md:grid md:grid-cols-2 m-6 md:my-4 md:mx-0 md:w-full">
        <div className="md:mr-4">
          <div>
            ...
            <p>
              €{Intl.NumberFormat("en-GB").format(data.price_per_night)} / night
            </p>
          ...
          <div className="md:bg-white md:border md:grid md:grid-cols-2 md:rounded-2xl md:p-8 md:gap-8 md:max-w-lg">
            ...
            <div>
              ...
              <p>
                {data.house_rules.map((item: String, index: number) => (
                  <span key={index}>
                    {item}
                    <br />
                  </span>
            ...
            <div>
              ...
              <p>
                {data.safety_information.map((item: String, index: number) => (
                  <span key={index}>
                    {item}
                    <br />
                  </span>
            ...
            <div>
              ...
              <p>
                {data.property_features.map((item: String, index: number) => (
                  <span key={index}>
                    {item}
                    <br />
                  </span>
            ...
            <div>
              ...
              <p>
                {data.property_services.map((item: String, index: number) => (
                  <span key={index}>
                    {item}
                    <br />
                  </span>
          ...
          <div className="flex flex-col gap-4 md:bg-white md:max-w-lg md:mt-4 md:border md:rounded-2xl md:p-8">
            ...
            <Link
              ...
              className="px-8 py-4 rounded-xl border border-buttonPrimary text-center mx-auto md:p-0 md:border-0 md:underline md:font-medium md:ml-0"
            >
              Show all reviews
            </Link>
          </div>
        ...
        <div className="fixed flex h-[80px] justify-center items-center bg-[#F0F0F0] border bottom-0 left-0 w-full md:hidden">
          <Link
            href={`${params.id}/request`}
            className="px-12 py-4 rounded-xl bg-accent hover:bg-[#068488c2] text-white"
          >
            Request
          </Link>
        </div>

...
async function BookingConfirmationPage({ params }: { params: { id: string } }) {
  ...
  const { data, error } = await supabase
    .from("bookings")
    .select(
      "start_date, end_date, guest_count, total_price, ...listings(title, city, country, images, price_per_night, cleaning_fee)"
    )
    .eq("id", params.id)
    .single();
  ...
  return (
    <div className="flex flex-col items-center h-[calc(100vh-256px)] mt-8 md:mt-24">
      ...
      <div className="flex flex-col gap-4 bg-white m-4 px-4 py-6 shadow-xl rounded-xl max-w-4xl">
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex justify-between">
            <p>
              €{data.price_per_night} x {nights} nights
            </p>
      ...
      <div className="flex flex-col justify-center gap-4">
        ...
        <Link
          href="/"
          className="bg-buttonPrimary hover:bg-buttonPrimaryHover text-white text-center py-4 rounded-lg max-w-md"
        >
          Return to home
        </Link>
      </div>

...
const BookingRequest: React.FC<BookingRequestProps> = ({
  ...
  const [nights, setNights] = useState(
    calculateNights(bookingData.startDate, bookingData.endDate)
  );
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === "startDate" || name === "endDate") {
      const updatedStartDate =
        name === "startDate" ? value : bookingData.startDate;
  ...
  const handleNext = async () => {
    ...
    if (isOverlap) {
      setErrorMessage(
        "This booking overlaps with another existing booking. Please choose different dates."
      );
      return;
    }
  ...
  return (
    <div
      className={`${className} md:max-w-lg md:bg-white md:border md:rounded-2xl md:p-8`}
    >
              ...
              <div className="relative flex flex-col p-2">
                ...
                >
                  {!isDropdownOpen && (
                    <p className="">
                      {guestCountState}{" "}
                      {guestCountState === 1 ? "guest" : "guests"}
                    </p>
                  )}
                  {isDropdownOpen ? (
                    <FaChevronUp
                      size={20}
                      className="items-center mx-auto pointer-events-none"
                    />
                  ) : (
                  ...
                  <div className="flex justify-between bg-background p-2 mx-2 rounded md:bg-white">
                    ...
                    <div className="flex gap-4">
                      ...
                      <button
                        ...
                        disabled={guestCountState >= maxGuests}
                      >
                        <FaPlus size={20} />
                      </button>
                    </div>

...
const FilterModal = ({ closeModal, toggleModal }: { closeModal: () => void; toggleModal: () => void }) => {
  ...
  return (
    <div className="z-50 absolute inset-0 flex flex-col place-self-center bg-background p-4 mt-12 mx-[-16px] h-screen md:h-fit md:mt-14 md:rounded-xl md:p-4 md:shadow-lg min-w-max max-w-3xl w-full md:max-w-lg md:border md:border-[#CCCCCC]">
       ...
       {/* Price range */}
        ...
        {/* Number of guests */}
        <div className="mt-4">
          <label className="ml-8 block mb-2 font-semibold">Number of guests</label>
          <div className="space-x-4 place-self-center">
            <button
              onClick={() => handleMaxGuestsChange(null)}
              className={`px-2 py-1 rounded-full border border-[#999999] min-w-16 ${filters.maxGuests === null ? 'bg-accent text-white border-0' : 'bg-background text-foreground'}`}
            >
              Any
            </button>
            <button
              onClick={() => handleMaxGuestsChange(1)}
              className={`px-2 py-1 rounded-full border border-[#999999] min-w-16 ${filters.maxGuests === 1 ? 'bg-accent text-white border-0' : 'bg-background text-foreground'}`}
            >
              1
            </button>
            <button
              onClick={() => handleMaxGuestsChange(2)}
              className={`px-2 py-1 rounded-full border border-[#999999] min-w-16 ${filters.maxGuests === 2 ? 'bg-accent text-white border-0' : 'bg-background text-foreground'}`}
            >
              2
            </button>
            <button
              onClick={() => handleMaxGuestsChange(4)}
              className={`px-2 py-1 rounded-full border border-[#999999] min-w-16 ${filters.maxGuests === 4 ? 'bg-accent text-white border-0' : 'bg-background text-foreground'}`}
            >
              4
            </button>
            <button
              onClick={() => handleMaxGuestsChange(8)}
              className={`px-2 py-1 rounded-full border border-[#999999] min-w-16 ${filters.maxGuests === 8 ? 'bg-accent text-white border-0' : 'bg-background text-foreground'}`}
            >
              8+
            </button>
          </div>
        ...
        {/* Property Features */}
        <div className="mt-4 ">
          <label className="block mb-2 font-semibold">Property Features</label>
          <div
            className={`space-y-2 mb-2 ${
              showMoreFeatures ? 'max-h-36 overflow-y-scroll' : ''
            }`}
          >
            {availableFeatures.slice(0, showMoreFeatures ? availableFeatures.length : 4).map((feature) => (
        ...
        <div className="">
          ...
          >
            {availableServices.slice(0, showMoreServices ? availableServices.length : 4).map((service) => (

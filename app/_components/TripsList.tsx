...
export default function TripsList({ userId, initialBookings }: TripsListProps) {
  ...
  }, [userId, initialBookings]); // Dependencies
        ...
        {(viewUpcoming ? upcomingBookings : pastBookings).map((booking) => (
          <div key={booking.id} className="flex flex-col gap-4 bg-white shadow-xl rounded-xl m-8 p-4 md:gap-8 md:p-8">
            ...
            {new Date(booking.end_date) < new Date() || new Date(booking.start_date) <= new Date() ? (
              ...
              <>
                <CancelBookingModal bookingId={booking.id} className={"flex-1 bg-buttonSecondary hover:bg-buttonSecondaryHover text-[#333333] py-2 rounded-lg"} />

...
export default function TripsList({ userId, initialBookings }: TripsListProps) {
  ...
  const pastBookings = bookings.filter(
    (booking) => new Date(booking.end_date) < new Date() || new Date(booking.start_date) <= new Date()
  );
  ...
  return (
    ...
    <div className="flex max-w-md w-full bg-buttonSecondary rounded-full">
      <button
        ...
        className={`flex-1 py-2 px-6 rounded-full ${viewUpcoming ? "bg-buttonPrimary text-white" : "bg-buttonSecondary text-[#333333]"}`}
      >
        Upcoming
      </button>
      <button
    ...
    <div className="w-full max-w-lg mb-24 md:max-w-xl">
      ...
      {(viewUpcoming ? upcomingBookings : pastBookings).length === 0 ? (
        <div className="flex flex-col gap-4 bg-white shadow-xl rounded-xl m-8 p-6 md:gap-8 md:p-8">
          <h2 className="font-semibold text-xl text-center">No Trips Found</h2>
          <p className="text-center">It looks like you haven't booked any trips yet. Find the perfect luxury escape today!</p>
          <Link href="/">
        ...
        (viewUpcoming ? upcomingBookings : pastBookings).map((booking) => (
          <div key={booking.id} className="flex flex-col gap-4 bg-white shadow-xl rounded-xl m-8 p-4 md:gap-8 md:p-8">
            <div>
              <p>Booking ID: {booking.id.split('-')[0]}</p>
              <p className="text-secondary">
                Booking Date: {new Date(booking.start_date).toLocaleDateString("en-US", { month: "long", day: "numeric" })} - {" "}
                {new Date(booking.end_date).toLocaleDateString("en-US", { month: "long", day: "numeric" })}
              </p>
            </div>
            <div className="flex gap-4">
              ...
              />
              <div>
                <h2 className="font-semibold md:text-2xl">{booking.listings.title}</h2>
            ...
            <div className="flex gap-8">
              {new Date(booking.end_date) < new Date() || new Date(booking.start_date) <= new Date() ? (
                <>
                  <button
    ...
    <ReviewModal isOpen={isReviewModalOpen} onClose={() => setReviewModalOpen(false)} listingId={selectedListingId!} userId={userId} />
  </>
  );
}